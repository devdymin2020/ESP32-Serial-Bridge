...
void setup() {
  ...
  if(debug) COM[DEBUG_COM]->println("\nWiFi connected");
  if(debug) COM[DEBUG_COM]->printf("local IP %s, port %d\n", WiFi.localIP().toString().c_str(), SERIAL0_TCP_PORT);
...
{  
...
#endif  
#ifdef PROTOCOL_TCP
  for(int num= 0; num < 3 ; num++)  //NUM_COM
  {
  ...
  for(int num= 0; num < 3 ; num++)  //NUM_COM
  {
    if(COM[num] != NULL)          
    {
      ...
      {
//        if(debug) COM[DEBUG_COM]->println("recv ok");
